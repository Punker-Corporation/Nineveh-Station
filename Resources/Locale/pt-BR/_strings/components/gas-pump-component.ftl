comp-gas-pump-ui-pump-status = Estado:
comp-gas-pump-ui-status-enabled = Sobre
comp-gas-pump-ui-status-disabled = Desligado
comp-gas-pump-ui-pump-set-rate = Instalar
comp-gas-pump-ui-pump-set-max = Máximo
comp-gas-pump-ui-pump-output-pressure = Pressão de saída (kPa):
comp-gas-pump-ui-pump-transfer-rate = Taxa de alimentação (L/seg):
comp-gas-pump-ui-needs-anchor = Proteja-o primeiro!
