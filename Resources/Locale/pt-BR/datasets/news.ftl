news-dataset-1 = Árvore presa em tajaran; bombeiros perplexos.
news-dataset-2 = Os tatus estão pedindo que os tamanduás sejam retirados do dicionário, alegando que eles vieram primeiro.
news-dataset-3 = Angel encontrou dança em cabeça de alfinete ordenado para parar; citado por incômodo público.
news-dataset-4 = Cartas afirmam que são melhores do que número; "Sempre foram".
news-dataset-5 = Canetas declaram lápis obsoletos: “O chumbo está morto”.
news-dataset-6 = Rock and Paper processa tesoura por discriminação.
news-dataset-7 = Steak admite em um livro franco que nunca gostou de sentar ao lado de batatas.
news-dataset-8 = A marmota parou de contar quantas vezes jogou a árvore: “Nunca mais”.
news-dataset-9 = "Beijo-beijo" não é mais o método preferido de busca de tajarans.
news-dataset-10 = Homem viaja 7.000 anos-luz para devolver lenço perdido: “Era meu favorito.”
news-dataset-11 = Uma nova pista de boliche que atira minimeteoros nos jogadores se tornou muito popular.
news-dataset-12 = Skrell se casa com computador; casamento assistido por 100 modems.
news-dataset-13 = Um chef relata sucesso ao usar uma gaita como ralador de queijo.
news-dataset-14 = A fundação inventou um lenço que diz “Deus te abençoe” depois que você espirra.
news-dataset-15 = O clone é acusado de posar para outros clones em uma foto da escola.
news-dataset-16 = O clone é acusado de roubar o prêmio de Funcionário do Mês dos demais clones.
news-dataset-17 = Mulher rouba estação com secador de cabelo; tripulantes amam novo estilo.
news-dataset-18 = Este lugar é para alugar.
news-dataset-19 = O cientista Skrell descobriu que o ábaco pode ser usado para secar toalhas.
news-dataset-20 = Enquete: Cheese Louise é eleita a melhor pizzaria de Tau Ceti.
news-dataset-21 = O gás do pântano acabou sendo a exalação de estrelas há muito desaparecidas - estrelas de cinema.
news-dataset-22 = Brócolis infectado é a arma preferida dos assassinos do sindicato.
news-dataset-23 = Os chefs descobriram que o brócolis é eficaz para cortar queijo.
news-dataset-24 = O brócolis causa irritabilidade em macacos.
news-dataset-25 = Uma bola de pelo gigante tem uma gramática perfeita, mas emite demais o som do “r”, dizem os linguistas.
news-dataset-26 = Enquete: As atualizações do Gibson Gazette costumam ser absurdas.
news-dataset-27 = Tajarans exigem pausas mais longas, bandejas mais limpas e ratos mais lentos.
news-dataset-28 = Enquete: 3 em cada 5 Skrells odeiam arte moderna.
news-dataset-29 = O cientista Skrell descobriu a gravidade ao cair de um lance de escadas.
news-dataset-30 = Pessoas de todo o mundo concordam: Tajarans ronronantes são Tajarans felizes.
news-dataset-31 = Do escritório de Sage Sammy: Uma palavra neste jornal está escrita ao contrário.
news-dataset-32 = Do escritório de Sage Sammy: É difícil ter muito espaço nas prateleiras.
news-dataset-33 = Do escritório de Sage Sammy: Vinho e amizade melhoram com a idade.
news-dataset-34 = Do escritório de Sage Sammy: As bolas de golfe têm principalmente elásticos dentro delas.
news-dataset-35 = Do escritório de Sage Sammy: Você não precisa enganar a todos, apenas as pessoas certas.
news-dataset-36 = Do escritório de Sage Sammy: Se você faz bagunça, você limpa.
news-dataset-37 = Do escritório de Sage Sammy: É mais fácil obter perdão do que permissão.
news-dataset-38 = Do escritório de Sage Sammy: Verifique os fatos antes de parecer estúpido.
news-dataset-39 = Do escritório de Sage Sammy: Você não pode esperar a burocracia passar.
news-dataset-40 = Do escritório de Sage Sammy: É melhor ceder do que exigi-la.
news-dataset-41 = Do escritório de Sage Sammy: Uma pessoa que adora gatos não pode ser má.
news-dataset-42 = Do escritório de Sage Sammy: Ajudar é o lado bom do controle.
news-dataset-43 = Do escritório de Sage Sammy: Dois pontos definem uma linha reta.
news-dataset-44 = Do escritório de Sage Sammy: Ler melhora a mente e eleva o espírito.
news-dataset-45 = Do escritório de Sage Sammy: É melhor mirar alto e errar do que mirar baixo e acertar.
news-dataset-46 = Do escritório de Sage Sammy: Os meteoros costumam atingir o mesmo local mais de uma vez.
news-dataset-47 = Tommy B. Seif diz: Olhe para os dois lados antes de embarcar no ônibus.
news-dataset-48 = Tommy B. Saif Sez: Segure-se; Súbita pára às vezes necessário
news-dataset-49 = Tommy B. Seif diz: Mantenha os dedos longe dos painéis móveis.
news-dataset-50 = Tommy B. Seif diz: Não há conversão à esquerda, exceto para ônibus.
news-dataset-51 = Tommy B. Seif diz: Retorne os assentos e as bandejas para a posição vertical correta.
news-dataset-52 = Tommy B. Seif diz: Não é permitido comer ou beber nas estações de ancoragem.
news-dataset-53 = Tommy B. Seif diz: Não aceite substitutos e não se deixe enganar por falsificações.
news-dataset-54 = Tommy B. Seif diz: Não remova esta etiqueta sob pena de lei.
news-dataset-55 = Tommy B. Seif diz: Sempre misture bem quando for instruído a fazê-lo.
news-dataset-56 = Tommy B. Seif diz: Tente ter despesas equivalentes a seis meses.
news-dataset-57 = Tommy B. Seif diz: O troco não é dado sem uma compra.
news-dataset-58 = Tommy B. Seif diz: Se você quebrou, você compra.
news-dataset-59 = Tommy B. Seif diz: As reservas devem ser canceladas 48 horas antes do evento para receberem um reembolso.
news-dataset-60 = Donuts: Existe alguma coisa que eles não possam fazer?
news-dataset-61 = Se os apitos são feitos de estanho, então de que são feitos os clarins?
news-dataset-62 = Brócolis acaba sendo uma colônia de pequenos alienígenas com pensamentos assassinos.

## Commented


# {{AFFECTED}} clerk first person able to pronounce '@*$%!'.
# {{AFFECTED}} delis serving boiled paperback dictionaries, 'Adjectives chewy' customers declare.
# {{AFFECTED}} weather deemed 'boring'; meteors and rad storms to be imported.
# Most {{AFFECTED}} security officers prefer cream over sugar.
# Palindrome speakers conference in {{AFFECTED}}; 'Wow!' says Otto.
# Question mark worshipped as deity by ancient {{AFFECTED}} dwellers.
# Spilled milk causes whole {{AFFECTED}} populace to cry.
# World largest carp patty at display on {{AFFECTED}}.
# Guy gets tattoo of Tau Ceti on chest '[pick(CentCom,star,starship,asteroid)] tickles most'.
# {{AFFECTED}} Baker Wins Pickled Crumpet Toss Three Years Running
# I Was Framed, jokes {{AFFECTED}} artist
# Mysterious Loud Rumbling Noises In {{AFFECTED}} Found To Be Mysterious Loud Rumblings
# Alien ambassador becomes lost on {{AFFECTED}}, refuses to ask for directions
# Survey: 80% Of People on {{AFFECTED}} Love Clog-Dancing
# {{AFFECTED}} Phonebooks Print All Wrong Numbers; Results In 15 New Marriages
# Tajaran Burglar Spotted on {{AFFECTED}}, Mistaken For Dalmatian
# Esoteric Verbosity Culminates In Communicative Ennui, {{AFFECTED}} Academics Note
# Boy Saves Tajaran From Tree on {{AFFECTED}}, Thousands Cheer
# Shipment Of Apples Overturns, {{AFFECTED}} Diner Offers Applesauce Special
# Spotted Owl Spotted on {{AFFECTED}}
